---
title: Agent Skills
description: Copie este bloco para o system prompt ou skills do seu agente.
---

Forneça aos seus agentes de IA o contexto completo, padrões de uso e assinaturas de API da biblioteca Catcher.

### Instalação via CLI

Se você utiliza a ferramenta de gerenciamento de skills, execute o comando abaixo:
...
Se preferir a instalação manual, copie o bloco abaixo e adicione-o ao **System Prompt** ou à seção de **Skills** do seu agente.
...
| Type                | Descrição                                         |
| ------------------- | ------------------------------------------------- |
| Result<T, E>        | Sucesso (ok) ou falha (err) [E, undefined] \| [undefined, T] |
| AsyncResult<T, E>   | Promise<Result<T, E>>                             |
| ResultSuccess<T, E> | Resultado de sucesso (type guard)                 |
| ResultFailure<T, E> | Resultado de falha (type guard)                   |
...
### catchError - Promise ou Função → Result
...
### catchErrorWithTimeout - com timeout
...
const [error, data] = result; // Estreitamento perfeito: if (error) -> data é undefined
if (error) { 
...
| { promise: Promise<T> | (() => Promise<T>); timeoutMs?: number; errorsToCatch?: ErrorClass[]; handler?: (error: any) => T | void }
